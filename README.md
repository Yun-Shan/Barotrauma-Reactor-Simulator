...
联合(2式)得：
每帧最多可变化的温度 = 10 * deltaTime = (新产生热量 - 涡轮输出) - (旧产生热量 - 涡轮输出)
                  = 新产生热量 - 旧产生热量
...
(注：如果有关注本人以前研究的旧版温控逻辑需要注意调节温度的定义发生了更改，以前是将调节温度固定为正负最大值，有超调的风险，而新版则是通过插值的方式直接插到目标值，一步到位不超调)

但是这个调节温度仍然需要进一步的限制，因为每帧的裂变目标值是上限为5 * deltaTime
这意味着如果 调节裂变 - 需求裂变 的绝对值如果超过 5 * deltaTime 太多就会导致振荡
因此我们需要提前降低调节温度的范围，这里定义一个降速阈值，根据(4.1式)：
(4.3)修正信号温差 = 16.66667 * clamp(这一帧和上一帧的温差 / 降速阈值, -1, 1)
这样当温差小于阈值时就会按比例降速，在防过冲、防振荡、高性能之间取得了较好的平衡。
这里给出一个降速阈值的参考值：50，经测试这个值的效果是比较好的

到此为止我们实现了固定负载变化(相当于负载是个频率较低的方波)的理论最优调整。
但如果只有这么简单的变化不如干脆用省油公式堆得了，现实是温控堆必须考虑更复杂的变化。
那么问题就来了，当变化复杂时以上的调整方式是会过冲的，原因还是目标值只能以每帧5 * deltaTime的速度变化，当负载反向变化时目标值的变化性能明显不足
...
下面通过预测温度对调节信号温度进行修正：
判断(7式)和(4.3式)，如果两者的结果是异号的，说明功率变化方向和负载变化方向是相反的，
